...
def eventhandler_eval(callback=None):
    ...
    for i in range(0, len(result)-1):
        if i != 0 and result[i] == "(" and result[i-1] != "*" and result[i-1] != "/" and result[i-1] != "+" and result[i-1] != "-":
            print(result[i], result[i-1])
            result = result[0:i] + "*" + result[i:len(result)]
            print(result)

    result_label["text"] = str(eval(result))
    return

def eventhandler_operators(op):
    global result_label
    if result_label["text"] == "0" and op == "(":
        result_label["text"] = op
        return
    else:
        result_label["text"] = result_label["text"] + op
        return

def eventhandler_del(callback=None):
    global result_label
    result_label["text"] = result_label["text"][0:len(result_label["text"])-1]
    return
...
def eventhandler_op_bracketL(callback=None):
    eventhandler_operators('(')
    return

def eventhandler_op_bracketR(callback=None):
    eventhandler_operators(')')
    return
...
root.bind("(", eventhandler_op_bracketL)
root.bind(")", eventhandler_op_bracketR)
root.bind("=", eventhandler_eval)
root.bind("<Return>", eventhandler_eval) # <Return> indicates the enter key
root.bind("Del", eventhandler_del)
root.bind("<BackSpace>", eventhandler_del)
...
ttk.Button(frm, text="Del", command=eventhandler_del).grid(column=2, row=5)
# Digits
...
ttk.Button(frm, text="(", command=eventhandler_op_bracketL).grid(column=0, row=5)
ttk.Button(frm, text=")", command=eventhandler_op_bracketR).grid(column=1, row=5)
